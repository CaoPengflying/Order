...
function getEmployees(){
	$.ajax({
		...
		//async:false,//同步加载，意味着必须加载完成才能继续后续操作，以保障在加载订单之前加载完班组成员
        success: function(result){
        	var grid = $("#datagrid_employees");
        	var data = JSON.parse(result);
       		employees = data.rows;
       		grid.datagrid({data:employees});
...
<body class="easyui-layout">
	...
	<div data-options="region:'east',title:'帮助',collapsible:true,collapsed:true,width:250" style="padding:10px;">
		<%@include file="../../inc/help/group/order.inc" %> 	
	</div>
	<div data-options="region:'center'" >
		<div class="easyui-accordion" style="height:100%" >
			<div id="div_orders" title="预订套餐" data-options="selected:false,iconCls:'icon-order'" style="height:398px;">
				<table  class="easyui-datagrid" id="datagrid_orders" data-options="toolbar:'#toolbar_order',fit:true,striped:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true,onCheck:SelectOrder,onUncheck:SelectOrder">
					<thead>
						<tr>
				...
				<table  class="easyui-datagrid" id="datagrid_employees" data-options="toolbar:'#toolbar_employee',fit:true,striped:true,checkOnSelect:true,selectOnCheck:false,singleSelect:true">
					<thead>
						<tr>
							...
							<th data-options="field:'placeIDs',width:140,align:'center',formatter:placeFormatter">送餐点</th>
							<th data-options="field:'lunch',width:100,align:'right'">中餐剩余</th>
							<th data-options="field:'dinner',width:100,align:'right'">晚餐剩余</th>
							<th data-options="field:'midnight',width:100,align:'right'">零点餐剩余</th>
						</tr>
					</thead>
				</table>
			</div>	
