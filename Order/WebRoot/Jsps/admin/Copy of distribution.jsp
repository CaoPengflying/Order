...
function getDistributions(){
	...
	$.post("/Order/OrderServlet",
		...
		var data = JSON.parse(result);
		table += "<table  cellspacing='0' cellpadding='3' padding='10' border='2' style='font-size:14px;align:center;margin:0 auto;border-collapse:collapse;'>";
		...
		if(data.rows.length != 0) {
			var length = 0;
			var place = length+1;
			var amount = data.rows[length].amount;
			var remark = data.rows[length].remark;
			if(place < data.total) {
				while(data.rows[length].placeID == data.rows[place].placeID) {
					if(data.rows[place].remark != "") {
						remark = addRemark(remark,data.rows[place].remark);
					}
					amount += data.rows[place].amount;
					place++;
				}
		}
			table += "<tr >";
			table += "<td style='font-size:16px;font-weight:bold;'>"+data.rows[length].placeName+"合计</td>";
			table += "<td></td>";
			table += "<td style='font-size:16px;font-weight:bold;text-align:center;'>("+amount+")</td>";
			table += "<td style='font-size:16px;font-weight:bold;'>"+remarkFormat(remark)+"</td>";
			table += "<td></td>";
			table += "</tr>";
			table += "<tr>";
			table += "<td></td>";
			table += "<td>"+data.rows[length].departmentName+"</td>";
			table += "<td style='text-align:center;'>"+data.rows[length].amount+"</td>";
			table += "<td>"+remarkFormat(data.rows[length].remark)+"</td>";
			table += "<td></td>";
			table += "</tr>";
				
				for(length=1; length<data.total; length++) {
					place = length+1;
					remark = data.rows[length].remark;
					amount = data.rows[length].amount;
					if(place != data.total) {
						while(data.rows[length].placeID == data.rows[place].placeID) {
						if(data.rows[place].remark != "") {
							remark = addRemark(remark,data.rows[place].remark);
						...
						if(data.rows[length].placeID != data.rows[length-1].placeID){
							table += "<tr >";
							table += "<td style='font-size:16px;font-weight:bold;'>"+data.rows[length].placeName+"合计</td>";
							table += "<td></td>";
							table += "<td style='font-size:16px;font-weight:bold;text-align:center;'>("+amount+")</td>";
				...
				table += "<tr>";
				table += "<td></td>";
				table += "<td style='font-weight:bold;text-align:center;'>"+data.footer.departmentName+"</td>";
				table += "<td style='font-weight:bold;text-align:center;'>"+data.footer.amount+"</td>";
				table += "<td></td>";
				table += "<td></td>";
				table += "</tr>";
				table += "</tbody>";
			...
			table += "</table>";
			document.getElementById("datagrid_orders").innerHTML = table;
		}
	); 
}	
...
function doPrint() {
	...
	window.showModalDialog("print.html?t="+Math.random(), printHtml,  "location:No;status:No;help:No;dialogWidth:800px;dialogHeight:600px;scroll:auto;");
}
